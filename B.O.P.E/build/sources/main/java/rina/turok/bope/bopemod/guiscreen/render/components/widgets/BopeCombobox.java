...
public class BopeCombobox extends BopeAbstractWidget {
	...
	public void render(int master_y, int separate, int mx, int my) {
		...
		if (is_open()) {
			...
			BopeDraw.draw_string(this.setting.get_current_value(), this.x + this.width - separate - this.font.get_string_width(this.setting.get_current_value(), this.smoth) - 2, this.save_y, ns_r, ns_g, ns_b, this.smoth);
		} else {
			BopeDraw.draw_string(this.combobox_name, this.x + 2, this.save_y, ns_r, ns_g, ns_b, this.smoth);
			BopeDraw.draw_string(this.setting.get_current_value(), this.x + this.width - separate - this.font.get_string_width(this.setting.get_current_value(), this.smoth) - 2, this.save_y, ns_r, ns_g, ns_b, this.smoth);
		}
