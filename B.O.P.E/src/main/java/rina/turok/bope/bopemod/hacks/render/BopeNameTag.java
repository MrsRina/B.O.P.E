...
public class BopeNameTag extends BopeModule {
	...
	public void draw(Entity entitys) {
		if (mc.getRenderManager().options != null) {
			...
			String name = entitys.getName() + " " + ChatFormatting.DARK_RED + Math.round(((EntityLivingBase) entitys).getHealth() / 2 + (entitys instanceof EntityPlayer ? ((EntityPlayer) entitys).getAbsorptionAmount() : 0));
